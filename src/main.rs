...
impl Default for AppConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            proxy_port: 10880, // Default Psiphon HTTP port
    ...
    fn bootstrap() -> Self {
        ...
        let mut state = Self {
            ...
            logs: vec![LogMessage {
                ...
                level: LogLevel::Info,
                text: "🖥️ System Boot: Chained tester loaded.".to_string(),
            }],
            total_configs: 0,
            working_configs: 0,
...
impl Drop for AppState {
    fn drop(&mut self) {
        self.stop_flag.store(true, Ordering::SeqCst);
        // REMOVED: Global taskkill for xray.exe. 
        // We only kill our own processes inside the test function to keep your V2rayN alive.
    }
}
                ...
                egui::ScrollArea::vertical().show(ui, |ui| {
                    match self.active_tab {
                        0 => {
                            ...
                            ui.checkbox(&mut self.config.test_configs_enabled, "✅ Enable Chained Tester (via Upstream)");
                            ui.horizontal(|ui| { ui.label("Test Timeout (Sec):"); ui.add(egui::DragValue::new(&mut self.config.testing_timeout_seconds).clamp_range(5..=120)); });
                            ui.horizontal(|ui| { ui.label("Max Concurrent Tests:"); ui.add(egui::DragValue::new(&mut self.config.max_concurrent_tests).clamp_range(1..=10)); });
                        }
                        1 => {
...
fn generate_chained_xray_config(link: &str, test_port: u16, psiphon_host: &str, psiphon_port: u16) -> Option<String> {
    ...
    // Upstream Outbound (Psiphon)
    let upstream = json!({
        "tag": "psiphon_upstream",
        "protocol": "http", // Psiphon local proxy is usually HTTP
    ...
    // CRITICAL FIX: Use 'detour' to chain the connection
    main_outbound["detour"] = json!("psiphon_upstream");
...
fn build_vmess_outbound(link: &str) -> Option<serde_json::Value> {
    ...
    // Handle remarks (#remark) at the end
    let b64_clean = b64.split('#').next()?.trim();
    let decoded = STANDARD.decode(b64_clean).ok()?;
...
fn build_shadowsocks_outbound(link: &str) -> Option<serde_json::Value> {
    ...
    // Case 2: Legacy base64 (all encoded)
...
fn test_config_chained(link: &str, test_port: u16, psiphon_host: &str, psiphon_port: u16, timeout_secs: u64, tx: &Sender<AppEvent>) -> bool {
    let json_config = match generate_chained_xray_config(link, test_port, psiphon_host, psiphon_port) {
        Some(c) => c,
        None => {
            // Log silently or ignore unsupported protocols
            return false;
        }
    };
    ...
    // OPTIMIZATION: Port Knocking (Wait for Xray to be ready)
    let start = Instant::now();
    let mut ready = false;
    while start.elapsed().as_secs() < 5 { // Wait max 5s
    ...
    // Using Google 204 for fast check
    let result = client.get("https://www.google.com/generate_204").send();
    let is_working = result.is_ok(); // If no error, it connected through the tunnel
    ...
    if is_working {
        let _ = tx.send(AppEvent::Log(LogLevel::Success, format!("🟢 WORKING: {}", link.split_at(50).0)));
    }
...
fn test_configs_concurrently(links: &[String], config: &AppConfig, tx: &Sender<AppEvent>, output_dir: &str) -> usize {
    ...
    let max_con = config.max_concurrent_tests.clamp(1, 10);
...
fn run_worker(config: AppConfig, channels_raw: String, stop: Arc<AtomicBool>, tx: Sender<AppEvent>) -> Result<()> {
    ...
    log_worker(&tx, LogLevel::Info, format!("🚀 Crawler Started | Engine: {:?} | Upstream: {}:{}", config.engine, config.proxy_host, config.proxy_port));
    ...
    loop {
                        ...
                        for block in blocks {
                            ...
                            if is_valid_date {
                                for m in regex.find_iter(block) {
                                    ...
                                    if clean_link.starts_with("tg://resolve") || clean_link.starts_with("tg://join") { continue; }
        ...
        if config.test_configs_enabled && !new_only.is_empty() {
            log_worker(&tx, LogLevel::Info, "🔍 Starting concurrent chained testing...".to_string());
